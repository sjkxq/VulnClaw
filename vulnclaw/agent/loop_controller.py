...
from vulnclaw.agent.llm_client import call_llm_auto, call_llm_auto_streaming
...
async def auto_pentest_streaming(
    ...
    *,
    ...
    """Like :func:`auto_pentest`, but with token-level streaming output.

    ``on_chunk(delta, full_text_so_far, is_finished)`` is invoked as the LLM
    emits tokens. Round boundaries still produce ``on_step`` callbacks so
    callers can emit framing events (e.g. "round N complete").
    """
    results: list[AgentResult] = []
    ...
    for round_num in range(1, max_rounds + 1):
        ...
        try:
            response_text = await call_llm_auto_streaming(
                agent, system_prompt, round_context, on_chunk=on_chunk
            )
            result.output = response_text
            agent.context.add_assistant_message(f"[Round {round_num} 分析] {response_text}")
            agent._finding_parser.parse(response_text)
...
async def persistent_pentest_streaming(
    ...
    *,
    ...
    """Streaming variant of :func:`persistent_pentest`."""
    cycle_results: list[PersistentCycleResult] = []
    ...
    while not should_stop:
        ...
        try:
            ...
            if getattr(agent.context.state, "task_constraints", None):
                ...
                if rendered:
                    constraints_block = f"\n\n{rendered}"
            results = await auto_pentest_streaming(
                agent,
                user_input=(
                    f"[Persistent Cycle {cycle_num}] 继续对目标 {agent.context.state.target or '未知'} 进行渗透测试。"
                    f"这是第 {cycle_num} 个周期，保持之前的所有发现继续深入。"
